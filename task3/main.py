#python -m streamlit run D:\Games\University\IT\task3\main.py
import random
import numpy as np
...
class Schelling:
    ...
    def __init__(self, size, empty, condition):
        self.size = size
        self.empty_ratio = empty
        self.condition = condition

        p = [(1 - empty) / 2, (1 - empty) / 2, empty]
...
st.title("Shelling model")

size = st.sidebar.slider("Number of objects", 100, 10000, 2500)
empty = st.sidebar.slider("Probability of empty", 0., 1., .1)
condition = st.sidebar.slider("Number of happy neighbours", 2, 8, 2)
iterations = st.sidebar.number_input("Iterations", 10)

schelling = Schelling(size, empty, condition)
